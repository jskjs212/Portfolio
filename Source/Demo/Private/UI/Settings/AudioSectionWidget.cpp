...
void UAudioSectionWidget::SyncUIWithUserSettings()
{
    if (UDemoUserSettings* UserSettings = UDemoUserSettings::GetDemoUserSettings())
    {
        ...
        for (FVolumeSettingsEntry& Entry : VolumeSettingsEntries)
        {
            const float* LoadedVolume = UserSettings->GetVolumeSetting(Entry.Category);
            check(LoadedVolume);

            const float SliderValue = (*LoadedVolume) * VolumeMaxValue;
            Entry.Slider->SetValue(SliderValue);
            Entry.Text->SetText(FText::AsNumber(FMath::RoundToInt(SliderValue)));
        }
